# Builds ..\BubbleChamber.exe — a single self-contained exe (web app + WebView2 SDK DLLs embedded as app.zip).
...
try {
    ...
    & $csc /nologo /target:winexe /platform:x64 /optimize+ `
        "/out:$(Join-Path $root 'BubbleChamber.exe')" `
    ...
    Get-Item (Join-Path $root 'BubbleChamber.exe') | Select-Object Name, Length, LastWriteTime
}
finally {
